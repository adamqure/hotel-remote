...
class SQLiteReservationDataAccessObject(ReservationDataAccessObject):
    def updateReservation(self, reservation: Reservation):
        ...
        try:
            ...
            cursor.execute(command, (str(reservation._id), jsonpickle.encode(reservation.startDate), jsonpickle.encode(reservation.endDate), reservation.numberOfGuests, reservation.state.value, reservation.confirmationNumber, str(reservation._id)))
            connection.commit()
    ...
    def createReservation(self, reservation: Reservation):
        ...
        try:
            ...
            cursor.execute(command, (str(reservation._id), jsonpickle.encode(reservation.startDate), jsonpickle.encode(reservation.endDate), reservation.numberOfGuests, reservation.state.value, reservation.confirmationNumber))
            connection.commit()
        except Exception as e:
